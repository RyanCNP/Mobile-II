...
async function getCEPsByFilter(
  ...
  offset: number = 0
    ...
    try {
        ...
        if (cep) {
            const cepLimpo = parseInt(cep.replace(/\D/g, ''));
            if (!isNaN(cepLimpo)) {
                query += " AND CEP = ?";
                params.push(cepLimpo);
            }
        }
        ...
        console.log("Erro ao filtrar CEPs: ", error);
        return [];
    }
}
...
export { Database, createTables, createCEP, getCEPs, getAllCEPsByUser, getAllCEPs, getCEPsByFilter, deleteCEP, updateCEP };